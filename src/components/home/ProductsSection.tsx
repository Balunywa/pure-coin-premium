...
const products = [
  { name: 'CrewvoPay', description: 'Mobile payments, simplified.', href: 'https://crewvopay.com', gridColor: 'rgba(59, 130, 246, 0.15)', borderGlow: 'rgba(59, 130, 246, 0.3)' },
  { name: 'CrewvoApp', description: 'Social fitness for competitors.', href: 'https://crewvoapp.com', gridColor: 'rgba(16, 185, 129, 0.15)', borderGlow: 'rgba(16, 185, 129, 0.3)' },
];
      ...
      <div className="section-container relative z-10">
        <div className="max-w-4xl mx-auto">
          <FadeIn className="mb-20">
            ...
            <p className="text-muted-foreground mt-6 max-w-xl leading-[1.7]">
            ...
            {products.map((product, index) => (
              <FadeIn key={product.name} delay={index * 0.1}>
                <motion.a
                  ...
                  onMouseEnter={(e) => { setHoveredIndex(index); e.currentTarget.style.borderColor = product.borderGlow; }}
                  onMouseLeave={(e) => { setHoveredIndex(null); e.currentTarget.style.borderColor = 'hsl(var(--border) / 0.5)'; }}
                  whileHover={{ y: -4 }}
                ...
                >
                  <div 
                    ...
                    style={{
                      backgroundImage: `linear-gradient(to right, ${product.gridColor} 1px, transparent 1px), linear-gradient(to bottom, ${product.gridColor} 1px, transparent 1px)`,
                      backgroundSize: '48px 48px',
                    }}
                  />
                  <div 
                    className="absolute inset-0 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                    style={{ background: `radial-gradient(circle at 50% 50%, ${product.gridColor}, transparent 70%)` }}
                  />
                  ...
                  <div className="relative">
                    <div className="flex items-start justify-between mb-6">
                      <div className="flex items-center gap-3">
                        ...
                        <p className="text-xs uppercase tracking-[0.25em] text-muted-foreground">Case study</p>
                      </div>
                      <motion.div
                    ...
                    <h3 className="text-3xl md:text-4xl font-medium mb-3 group-hover:text-foreground transition-colors">{product.name}</h3>
                    <p className="text-lg text-muted-foreground group-hover:text-muted-foreground/80 transition-colors max-w-2xl">{product.description}</p>
