...
const technologies = [
  { id: 'frontend', category: 'Interface Layer', icon: Code2, color: '#3B82F6', freq: '3.4 Ghz', stack: ['React', 'Next.js', 'TypeScript', 'Tailwind'] },
  { id: 'backend', category: 'Computation Layer', icon: Cpu, color: '#10B981', freq: '800ms', stack: ['Node.js', 'Python', 'GraphQL', 'PostgreSQL'] },
  { id: 'infra', category: 'Global Network', icon: Globe, color: '#8B5CF6', freq: '99.99%', stack: ['Azure', 'Docker', 'Kubernetes', 'Terraform'] },
];
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-20">
      ...
      {horizontalLines.map((y, i) => (
        <motion.div key={`h-${i}`} className="absolute h-[1px] w-8 bg-white shadow-[0_0_5px_white]" style={{ top: `${y}%`, left: '-10%', backgroundColor: color }}
          animate={{ left: ['-10%', '110%'] }}
          transition={{ duration: Math.random() * 2 + 2, repeat: Infinity, ease: "linear", delay: Math.random() * 5, repeatDelay: Math.random() * 3 }}
        />
      ))}
      {verticalLines.map((x, i) => (
        <motion.div key={`v-${i}`} className="absolute w-[1px] h-8 bg-white shadow-[0_0_5px_white]" style={{ left: `${x}%`, top: '-10%', backgroundColor: color }}
          animate={{ top: ['-10%', '110%'] }}
          transition={{ duration: Math.random() * 2 + 2, repeat: Infinity, ease: "linear", delay: Math.random() * 5, repeatDelay: Math.random() * 3 }}
        />
      ))}
...
const CircuitCard = ({ tech, index }: { tech: typeof technologies[0], index: number }) => {
  ...
  return (
    ...
    >
      <div className="absolute -inset-[1px] rounded-xl bg-gradient-to-r from-transparent via-white/20 to-transparent opacity-0 group-hover:opacity-100 transition duration-500 blur-sm"
      ...
      <div className="relative bg-[#0A0A0A] border border-white/5 rounded-xl overflow-hidden h-full">
        ...
        <motion.div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent -skew-x-12"
          animate={{ x: ['-100%', '200%'] }}
        ...
        <div className="relative p-6 flex items-center justify-between gap-6 backdrop-blur-[1px]">
          <div className="flex items-center gap-5">
            <div className="relative w-14 h-14 flex items-center justify-center rounded-lg bg-white/5 border border-white/10 group-hover:border-white/20 transition-colors">
              ...
              <motion.div className="absolute inset-0 border-t border-r border-transparent rounded-lg"
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                ...
                <span className="px-1.5 py-0.5 rounded text-[9px] font-mono bg-white/5 text-white/50 border border-white/5">{tech.freq}</span>
              </div>
              <div className="flex gap-2 text-sm text-white/40">{tech.stack.join(' • ')}</div>
            </div>
          </div>
          <div className="hidden md:flex flex-col items-end gap-1">
            <div className="flex gap-1 h-3 items-end">
              {[1, 2, 3].map((bar) => (
                <motion.div key={bar} className="w-1 bg-white/20 rounded-sm"
            ...
            <span className="text-[10px] text-white/30 uppercase tracking-widest font-mono">Live</span>
          </div>
        </div>
      </div>
    </motion.div>
  ...
  return (
    <div className="relative w-full h-16 flex justify-center overflow-hidden">
      <div className="w-[2px] h-full bg-white/5 relative">
        <motion.div className="absolute top-0 w-full bg-gradient-to-b from-transparent via-white to-transparent shadow-[0_0_20px_2px_var(--cable-color)]"
          style={{ height: '60%', opacity: 0.8, '--cable-color': color } as any}
          animate={{ top: ['-100%', '200%'] }}
          transition={{ duration: 1.5, repeat: Infinity, ease: "circIn" }}
        />
        <motion.div className="absolute top-0 w-full bg-white/50" style={{ height: '20%' }}
          animate={{ top: ['-100%', '200%'] }}
      ...
      <div className="max-w-3xl w-full mx-auto px-6 relative z-10">
        <div className="text-center mb-20">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }}
            ...
            <span className="text-xs font-mono text-white/60 tracking-widest uppercase">System Architecture v2.0</span>
          </motion.div>
          <h2 className="text-5xl font-bold text-white mb-6 tracking-tight">The Neural Grid.</h2>
          <p className="text-white/40 max-w-lg mx-auto text-lg">A fully reactive, event-driven infrastructure that thinks before it acts.</p>
        </div>
        ...
        <div className="flex flex-col">
          ...
          {technologies.map((tech, index) => (
            <React.Fragment key={tech.id}>
              ...
              {index < technologies.length - 1 && <DataCable color={technologies[index + 1].color} />}
            </React.Fragment>
          ))}
